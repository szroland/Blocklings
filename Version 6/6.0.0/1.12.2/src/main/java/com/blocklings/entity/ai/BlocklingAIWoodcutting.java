...
public class BlocklingAIWoodcutting extends BlocklingAIGatherBase
{
    ...
    boolean canExecute()
    {
        ...
        else if (blockling.getTask() != Task.CHOP)
        {
            return false;
        }
    ...
    private boolean tryChopTarget()
    {
        ...
        if (blockling.getBlockBreakTimer() >= blockling.getChoppingInterval())
        {
        ...
        {
            int progress = (int)(((float)(blockling.getBlockBreakTimer()) / (float)blockling.getChoppingInterval()) * 9.0f);
