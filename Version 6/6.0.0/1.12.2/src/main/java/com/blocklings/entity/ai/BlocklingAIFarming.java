...
public class BlocklingAIFarming extends BlocklingAIGatherBase
{
    ...
    boolean canExecute()
    {
        ...
        else if (blockling.getTask() != Task.FARM)
        {
            return false;
        }
    ...
    private boolean tryHarvestTarget()
    {
        ...
        if (blockling.getBlockBreakTimer() >= blockling.getFarmingInterval())
        {
            harvestBlock();
            blockling.stopMining();
            world.sendBlockBreakProgress(blockling.getEntityId(), targetPos, -1);
        ...
        {
            int progress = (int)(((float)(blockling.getBlockBreakTimer()) / (float)blockling.getFarmingInterval()) * 9.0f);
