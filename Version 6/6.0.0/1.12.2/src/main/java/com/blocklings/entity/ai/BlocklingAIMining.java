...
public class BlocklingAIMining extends BlocklingAIGatherBase
{
    ...
    boolean canExecute()
    {
        ...
        else if (blockling.getTask() != Task.MINE)
        {
            return false;
        }
    ...
    private boolean tryMineTarget()
    {
        ...
        if (blockling.getBlockBreakTimer() == 0 && blockling.miningAbilities.isAbilityAcquired(AbilityHelper.brittleBlock))
        {
        ...
        if (blockling.getBlockBreakTimer() >= blockling.getMiningInterval())
        {
            mineTarget();
            blockling.stopMining();
            world.sendBlockBreakProgress(blockling.getEntityId(), targetPos, -1);
        ...
        {
            int progress = (int)(((float)(blockling.getBlockBreakTimer()) / (float)blockling.getMiningInterval()) * 9.0f);
