...
public class EntityBlockling extends EntityTameable implements IEntityAdditionalSpawnData {
	...
	public void openGui(EntityPlayer player) {
		if (world.isRemote) {	
			if (guiID == 0) NetworkHelper.sync(world, new OpenGuiMessage(getEntityId()));
			if (guiID == 1) player.openGui(Blocklings.instance, guiID, world, getEntityId(), 0, 0);  
		} else {
			if (guiID == 0) player.openGui(Blocklings.instance, guiID, world, getEntityId(), 0, 0);  
			if (guiID == 1) NetworkHelper.sync(world, new OpenGuiMessage(getEntityId()));
		}
	}
