...
public class EntityBlockling extends EntityTameable implements IEntityAdditionalSpawnData
{
    ...
    public Guard guard = Guard.NOGUARD;
    public State state = State.WANDER;

    public int skillPoints = 0;
    public int combatLevel = 1, miningLevel = 1, woodcuttingLevel = 1, farmingLevel = 1;
    public int combatXp = 0, miningXp = 0, woodcuttingXp = 0, farmingXp = 0;
    public int attackInterval = 10, miningInterval = 20, choppingInterval = 20, farmingInterval = 20;
    public int attackTimer = -1, blockBreakTimer = -1;
    public EnumHand attackingHand = EnumHand.MAIN_HAND;

    public byte autoswitchID = 0;
    ...
    protected void entityInit()
    {
        ...
        if (!world.isRemote)
        {
            do
            {
                scale = 1.0f + ((float) rand.nextGaussian() / 15.0f);
            }
    ...
    protected void initEntityAI()
    {
        ...
        this.targetTasks.addTask(6, aiHunt);
    }

    private void updateAI()
    {
        setSitting(state == State.STAY);
    }
    ...
    public boolean hasCorrectToolForJob(EnumHand hand)
    {
        ...
        else if (task == Task.CHOP)
        {
            return hasAxe(hand);
        }
        else if (task == Task.FARM)
        {
            return hasHoe(hand);
        }
    ...
    public void readEntityFromNBT(NBTTagCompound c)
    {
        ...
        task = Task.getFromID(c.getInteger("TaskID"));
        guard = Guard.getFromID(c.getInteger("GuardID"));
        state = State.getFromID(c.getInteger("StateID"));
    ...
    public void readSpawnData(ByteBuf buf)
    {
        ...
        task = Task.getFromID(buf.readInt());
        guard = Guard.getFromID(buf.readInt());
        state = State.getFromID(buf.readInt());
...
//                {
//                    List<Entity> entity = this.world.getEntitiesWithinAABB(Entity.class, getEntityBoundingBox().expand(2, 2, 2).expand(-2, -2, -2), null);
                ...
                else // Is tamed
                {
                    if (player == getOwner())
                    {
                        ...
                        else
                        {
                            if (!world.isRemote)
                            {
                                setState(state == State.STAY ? State.FOLLOW : State.STAY);
                                playSound(SoundEvents.BLOCK_LAVA_POP, 0.25f, 2.0f);
                            }
                        }
                    }
                }
...
//        }
    ...
    private void checkTimers()
    {
        ...
        {
            incrementBlockBreakTimer();
            if (blockBreakTimer > getCorrectInterval())
            {
                stopMining();
            }
        }
    ...
    {
        setBlockBreakTimer(0);
    }

    public void stopMining()
    {
        setBlockBreakTimer(-1);
    }

    public boolean isMining()
    {
        return blockBreakTimer!= -1;
    }
    ...
    public boolean isSetToAttack() { return (task == Task.HUNT || guard == Guard.GUARD || getAttackTarget() != null); }
    ...
    public boolean isUsingPickaxeRight() { return hasPickaxe(EnumHand.MAIN_HAND) && (task == Task.MINE); }
    public boolean isUsingPickaxeLeft() { return hasPickaxe(EnumHand.OFF_HAND) && (task == Task.MINE); }

    public boolean isUsingAxeRight() { return hasAxe(EnumHand.MAIN_HAND) && (task == Task.CHOP); }
    public boolean isUsingAxeLeft() { return hasAxe(EnumHand.OFF_HAND) && (task == Task.CHOP); }

    public boolean isUsingHoeRight() { return hasHoe(EnumHand.MAIN_HAND) && (task == Task.FARM); }
    public boolean isUsingHoeLeft() { return hasHoe(EnumHand.OFF_HAND) && (task == Task.FARM); }

    private void tryRemoveAttackTarget()
    {
        if (task != Task.HUNT && guard != Guard.GUARD)
        {
            setAttackTarget(null);
        }
    }
    ...
    private void checkBonusStats()
    {
        ...
        int miningIntervalValue;
        int choppingIntervalValue;
        int farmingIntervalValue;

        if (hasPickaxe()) miningIntervalValue = calcBonusMiningSpeedFromLevel(miningLevel) - (miningAbilities.isAbilityAcquired(AbilityHelper.hasteMining) ? 10 : 0);
        else miningIntervalValue = 0;
        if (hasAxe()) choppingIntervalValue = calcBonusMiningSpeedFromLevel(woodcuttingLevel) - (woodcuttingAbilities.isAbilityAcquired(AbilityHelper.hasteWoodcutting) ? 10 : 0);
        else choppingIntervalValue = 0;
        if (hasHoe()) farmingIntervalValue = calcBonusMiningSpeedFromLevel(farmingLevel) - (farmingAbilities.isAbilityAcquired(AbilityHelper.hasteFarming) ? 10 : 0);
        else farmingIntervalValue = 0;

        if (!mainHandEmpty && !offHandEmpty)
        {
            if (hasPickaxe(EnumHand.MAIN_HAND) && hasPickaxe(EnumHand.OFF_HAND))
            {
                setMiningInterval(miningIntervalValue / 2);
            }
            else if (hasAxe(EnumHand.MAIN_HAND) && hasAxe(EnumHand.OFF_HAND))
            {
                setChoppingInterval(choppingIntervalValue / 2);
            }
            else if (hasHoe(EnumHand.MAIN_HAND) && hasHoe(EnumHand.OFF_HAND))
            {
                setFarmingInterval(farmingIntervalValue / 2);
            }
        }
    }
    ...
    public void setTask(Task value)
    {
        ...
        {
            task = value;
            NetworkHelper.sync(world, new TaskIDMessage(value, getEntityId()));
    ...
    {
        int id = guard.id + 1;
        if (id > Guard.values().length)
        {
            id = 1;
        }

        setGuard(Guard.getFromID(id));
    }
    public void setGuard(Guard value)
    {
        ...
        {
            guard = value;
            NetworkHelper.sync(world, new GuardIDMessage(value, getEntityId()));
            tryRemoveAttackTarget();
            updateAI();
        }
    }
    ...
    public void setState(State value)
    {
        ...
        {
            state = value;
            NetworkHelper.sync(world, new StateIDMessage(value, getEntityId()));
            updateAI();
        }
    }
    ...
    {
        if (!getCustomNameTag().equals(value))
        {
    ...
    public void setAnimationState(AnimationState value)
    {
        if (animationState != value)
        {
            ...
            if (!world.isRemote) NetworkHelper.sync(world, new AnimationStateMessage(value, getEntityId()));
        }
    }
    ...
    public void setAttackTimer(int value)
    {
        ...
        {
            attackTimer = value;
            if (!world.isRemote) NetworkHelper.sync(world, new AttackTimerMessage(value, getEntityId()));
        }
    }
    ...
    public int getBlockBreakTimer()
    {
        return blockBreakTimer;
    }
    public void incrementBlockBreakTimer()
    {
        setBlockBreakTimer(blockBreakTimer + 1);
    }
    public void setBlockBreakTimer(int value)
    {
        if (blockBreakTimer!= value)
        {
            blockBreakTimer = value;
            NetworkHelper.sync(world, new BlockBreakTimerMessage(value, getEntityId()));
        }
    }
    ...
    public void setAutoswitchID(byte value)
    {
        if (autoswitchID != value)
        {
            autoswitchID = value;
